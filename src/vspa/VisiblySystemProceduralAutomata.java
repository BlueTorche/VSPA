...
public class VisiblySystemProceduralAutomata {
    ...
    public VisiblySystemProceduralAutomata(VSPAAlphabet alphabet) {
    ...
    public boolean accepts(String word) {
        if (!alphabet.getCallFromProcedural(startingAutomaton.getProceduralSymbol()).equals(String.valueOf(word.charAt(0)))) {
            return false;
        }

        Set<Map.Entry<VRA_State, String>> currentStates = Set.of(
            new AbstractMap.SimpleEntry<>(startingAutomaton.getInitalState(), startingAutomaton.getProceduralSymbol()));
        List<Set<Map.Entry<VRA_State, String>>> stack = new ArrayList<>();
        ...
        for (int i = 1; i < word.length() - 1 ; i++) {
            ...
            if (alphabet.getInternalSymbols().contains(symbol)) {
                ...
                Set<Map.Entry<VRA_State, String>> possibleNextState = stack.removeLast();
                for (Map.Entry<VRA_State, String> pair : currentStates) {
                    VRA_State state = pair.getKey();
        ...
        if (alphabet.getReturnFromCallSymbol(alphabet.getCallFromProcedural(startingAutomaton.getProceduralSymbol())).equals(String.valueOf(word.charAt(word.length() - 1))) &&
                stack.isEmpty()) {
            for (Map.Entry<VRA_State, String> pair : currentStates) {
